...
class InBatchFourierLoss(torch.nn.Module):
    def __init__(self: Self,
                 ...
                 scaler: Callable | None = None,) -> None:
        super().__init__()
    ...
    def get_image_momentum(self: Self,
                           n_embeddings: int,
    ...
    def get_samples(self: Self, item_embeddings: torch.Tensor, sampling_size: int) -> torch.Tensor:
    ...
    def compute_cdf_image(self: Self, x: torch.Tensor, mask: torch.Tensor | None = None, 
                          n_elems: torch.Tensor | None = None) -> torch.Tensor:
        return compute_cdf_image(x, self.n_harmonics, mask, n_elems)
    
    def compute_new_image(self: Self, embeddings: torch.Tensor, 
                          samples: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        for batch in batches:
            ...
            scores: torch.Tensor = torch.einsum("...h,ih->...i", \
                                    norm_embedding, part_item_embeddings)
            ...
            curr_image: torch.Tensor = self.compute_cdf_image(scores, casted_mask, curr_n_elems)
    ...
    def get_cdf_image(self: Self,
        ...
        result_image: torch.Tensor = image_momentum * new_image + \
                                (1.0 - image_momentum) * self.cdf_image
    ...
    def get_raw_scores(self: Self, embeddings: torch.Tensor, positive_labels: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        pos_scores: torch.Tensor = self.get_raw_scores(model_embeddings, positive_labels)
